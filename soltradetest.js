...
let first_message =
  "https://dexscreener.com/solana/32awdgby1jbtjvzctscvrlizz3ygjgxbvjanypyjc1by";
let second_message = "https://twitter.com/search?q=bitcoin&src=typed_query";
// sendMessageToTelegram(first_message + " " + second_message);
fetchSolanaPairData("BE1iUXqtS3cAPtmGardCceSvpyE3CcRur1DNwFxQbt9R");
function fetchSolanaPairData(pairAddress) {
...
function LargestAccountsCheck(token_mint) {
  return __awaiter(this, void 0, void 0, function* () {
    yield new Promise((resolve) => setTimeout(resolve, 10000)); // Delay for 10 seconds
    ...
    for (let i = 0; i < info.value.length; i++) {
      let consider = true;
      if (i === 0 || i === 1) {
        let largestAccountInfo = yield connection.getParsedAccountInfo(
